...
const MODE2_RULES = [
  ...
  },
  {
    name:"하이드라섹산10mg",
    type:"dual",
    ageText:a=> a<0.25 ? "3개월 미만 금기" : a>=12 ? "만 12세 이상 소아용법 없음" : "3개월~12세 투여 가능",
    weightText:w=> `${round2(Math.min(w*1.5,100))}mg tid (${round2(Math.min(w*0.15,10))}포, 1일 ${round2(Math.min(w*4.5,300))}mg, 상한 ${round2(w*6)}mg/d)`
  }
].sort((a,b)=>collator.compare(a.name,b.name));
...
const MODE3_RULES = [
  ...
  { name:"포리부틴드라이시럽", calc:a=> a>=5?"10ml tid":a>=1?"5ml tid":a>=0.5?"5ml bid":"2.5ml bid-tid" },
  { name:"포리부틴정", calc:a=> a>=15?"1~2정 tid (100-200mg tid, 라벨 성인용량)":a>=5?"0.62정 tid (62.4mg, 약 1/2정)":a>=1?"0.31정 tid (31.2mg, 분할불가→드라이시럽)":a>=0.5?"0.31정 bid (31.2mg, 분할불가→드라이시럽)":"0.16정 bid-tid (15.6mg, →드라이시럽)" },
...
const MODE_COMPOSITION_HINTS = {
  ...
  "포리부틴정": "Trimebutine maleate\n100 mg / tab (Trimebutine 76.95 mg)",
  "하이드라섹산10mg": "Racecadotril\n10 mg / g (10 mg / 포)",
  "씨투스건조시럽": "Pranlukast\n100 mg / g",
...
const MODE2_RESULT_HINTS = {
  ...
  "포리부틴드라이시럽": "5Y- : 10ml tid\n\n1-5Y : 5ml tid\n\n0.5-1Y : 5ml bid\n\n-0.5Y : 2.5ml bid-tid",
  "포리부틴정": "성인: 트리메부틴말레산염으로서 1회 100-200mg 1일 3회 식전\n\n[소아용량은 라벨에 없음 - 드라이시럽 환산]\n정제는 트리메부틴말레산염, 드라이시럽은 트리메부틴(유리염기) 표기\n말레산염 → 염기 환산계수 0.7695 (387.47/503.54)\n1정 = 말레산염 100mg = 트리메부틴 76.95mg = 드라이시럽 16.0mL\n\n5Y↑ : 10mL tid → 0.62정 tid\n1-5Y : 5mL tid → 0.31정 tid\n0.5-1Y : 5mL bid → 0.31정 bid\n-0.5Y : 2.5mL bid-tid → 0.16정 bid-tid\n\n1/2정 미만은 분할 부정확 → 드라이시럽 권장",
  "하이드라섹산10mg": "3개월 이상 ~ 만 12세 미만\n경구용 수분보급제와 함께 투여\n\n1회 1.5mg/kg, 1일 3회 일정 간격\n1일 최대 6mg/kg, 1주 최대 42mg/kg\n1회 최대 100mg\n\n정상 배변 2회까지 투여\n5일 이상 지속시 의사 조언\n7일 초과 금지, 장기투여 비권장\n3개월 미만 금기",
...
const MODE2_HEALTH_KR_URLS = {
  ...
  "포리부틴드라이시럽": "https://www.health.kr/searchDrug/result_drug.asp?drug_cd=A11A0500A0120",
...
function auditMode3Dual(rule, ctx) {
  ...
  if (ageAudit.text === "금기") return { status: "ex", text: "금기 (나이기준)" };
...
function normalizeMode3Dose(rule, dose) {
  ...
  if (rule.name === "하이드라섹산10mg") return dose * 10;
  return dose;
}
