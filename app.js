...
const MODE3_RULES = [
  ...
  { name:"포타겔현탁액", calc:a=> a>=12?"성인으로간주 20ml tid":a>=2?"3일간 13.3-20ml tid → 이후 4일간 13.3ml tid":"사용금기" },
...
const MODE2_RESULT_HINTS = {
  ...
  "포리부틴드라이": "5Y- : 10ml tid\n\n1-5Y : 5ml tid\n\n0.5-1Y : 5ml bid\n\n-0.5Y : 2.5ml bid-tid",
  "포타겔현탁액": "3일 동안 13.3ml-20ml(2-3g) tid\n\n이후 4일 동안 13.3ml(2g) tid",
  ...
  MERGED_MODE2_RULES.forEach(m => {
    ...
    if (m.type === "dual") {
      ...
      const weightLine = (weight === null || weight <= 0)
        ? "진정보조: 몸무게 입력 필요"
        : `진정보조: ${m.weightText(weight)}`;
      const separator = (m.name === "두드리진시럽(=이치리진)" || m.name === "아디팜정") ? "\n" : "\n\n";
    ...
    hasAgeSelected = true;
    const ageForRule = (m.name === "액티피드시럽" || m.name === "뮤테란과립200" || m.name === "페니라민정") ? agePrecise : age;
...
function auditMode3Age(rule, ctx) {
  const ruleAge = (rule.name === "액티피드시럽" || rule.name === "뮤테란과립200" || rule.name === "페니라민정") ? ctx.agePrecise : ctx.age;
...
function auditTextAgeRule(rule, recommendation, ctx) {
  ...
  if (rule.name === "뮤테란과립200" || rule.name === "포타겔현탁액" || rule.name === "듀락칸이지시럽" || rule.name === "루키오 / 싱귤레어") {
...
function getMode2NameCellHtml(name) {
  ...
  return link
    ? `<a class="med-link" href="${escapeAttr(link)}" target="_blank" rel="noopener noreferrer">${escapeHtml(name)}</a>`
    : `<span class="med-name-text">${escapeHtml(name)}</span>`;
}
